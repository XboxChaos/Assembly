...
namespace Blamite.Blam.ThirdGen.LanguagePack
{
    ...
    {
        private StructureLayout _layout;
        ...
        {
            var values = new StructureValueCollection();
            var rangeValues = Ranges.Select(r => r.Serialize()).ToArray();
        ...
        private void Load(IReader reader)
        {
            ...
            var values = StructureReader.ReadStructure(reader, _layout);
            
            var rangeValues = values.GetArray("language ranges");
