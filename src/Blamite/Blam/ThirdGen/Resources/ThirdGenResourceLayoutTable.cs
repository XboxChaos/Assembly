...
namespace Blamite.Blam.ThirdGen.Resources
{
    ...
    {
        public ThirdGenResourceLayoutTable(StructureValueCollection values, IReader reader, FileSegmentGroup metaArea, EngineDescription buildInfo)
        {
        ...
        private void LoadFileReferences(StructureValueCollection values, IReader reader, FileSegmentGroup metaArea, EngineDescription buildInfo)
        {
            ...
            var layout = buildInfo.Layouts.GetLayout("external cache file table entry");
        ...
        private void LoadPageInfo(StructureValueCollection values, IReader reader, FileSegmentGroup metaArea, EngineDescription buildInfo, ThirdGenCacheFileReference[] fileReferences)
        {
            int count = (int)values.GetInteger("number of raw pages");
            uint address = values.GetInteger("raw page table address");
            var layout = buildInfo.Layouts.GetLayout("raw page table entry");
            var entries = ReflexiveReader.ReadReflexive(reader, count, address, layout, metaArea);
        ...
        private void LoadSegments(StructureValueCollection values, IReader reader, FileSegmentGroup metaArea, EngineDescription buildInfo, ThirdGenResourcePage[] pages)
        {
            int count = (int)values.GetInteger("number of raw segments");
            uint address = values.GetInteger("raw segment table address");
            var layout = buildInfo.Layouts.GetLayout("raw segment table entry");
            var entries = ReflexiveReader.ReadReflexive(reader, count, address, layout, metaArea);
