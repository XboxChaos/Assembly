...
namespace Blamite.Blam.ThirdGen.Structures
{
    ...
    public class ThirdGenZoneSet : IZoneSet
    {
        ...
        {
            _activeResources.Length = Math.Max(_activeResources.Length, (int)index.Index + 1);
            _activeResources[(int)index.Index] = activate;
        }
        ...
        public bool IsResourceActive(DatumIndex index)
        {
            if (index.Index >= _activeResources.Count)
                return false;
            return _activeResources[(int)index.Index];
        }
        ...
        {
            _activeTags.Length = Math.Max(_activeTags.Length, (int)index.Index + 1);
            _activeTags[(int)index.Index] = activate;
        }
        ...
        public bool IsTagActive(DatumIndex index)
        {
            if (index.Index >= _activeTags.Count)
                return false;
            return _activeTags[(int)index.Index];
        }
        ...
        private void SaveBitArray(BitArray bits, string countName, string addressName, MetaAllocator allocator, IStream stream, ReflexiveCache<int> cache, StructureValueCollection values)
        {
            ...
            int[] ints = new int[((bits.Length + 31) & ~31) / 32];
            bits.CopyTo(ints, 0);
        ...
        {
            int oldCount = (int)values.GetInteger(countName);
