...
namespace Blamite.IO
{
	...
	public class ContainerReader
	{
		...
		private Stack<long> _blockEnds = new Stack<long>();
        ...
        /// The <see cref="BlockID"/>, <see cref="BlockName"/>, <see cref="BlockSize"/>, and <see cref="BlockVersion"/> properties will be updated to hold information about the block.
		/// </summary>
		...
		/// <summary>
		/// Ascends from the current block. The next block to be read will be the first block after the current block's last child block.
		/// </summary>
		...
		const int BlockHeaderSize = 9; // uint32 + int32 + byte
	}
}
