		...
		public void RefreshEditor(MetaReader.LoadType type)
		{
			...
			switch (type)
			{
				case MetaReader.LoadType.File:
					...
					headerOffset = (uint)_tag.RawTag.HeaderLocation.AsOffset();
					break;
...
#else
                    var metaUpdate = new MetaWriter(stream, (uint)_tag.RawTag.HeaderLocation.AsOffset(), (uint)_tag.RawTag.MetaLocation.AsOffset(), _cache, _buildInfo, type,
			...
			else if (_rteProvider != null)
			{
				using (IStream metaStream = _rteProvider.GetMetaStream(_cache))
				{
					if (metaStream != null)
					{
						...
						var metaUpdate = new MetaWriter(metaStream, _tag.RawTag.HeaderLocation.AsPointer(), _tag.RawTag.MetaLocation.AsPointer(), _cache, _buildInfo, type,
	...
	//	}
		...
		private void ReallocateCommand_Executed(object sender, ExecutedRoutedEventArgs e)
		{
			...
			using (var stream = _fileManager.OpenReadWrite())
			{
				...
				var metaUpdate = new MetaWriter(stream, (uint)_tag.RawTag.HeaderLocation.AsOffset(), (uint)_tag.RawTag.MetaLocation.AsOffset(), _cache, _buildInfo,
