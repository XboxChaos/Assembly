	...
	public partial class PatchControl
	{
		...
		private void btnCreatePatch_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				try
				{
					...
					var originalFile = CacheFileLoader.LoadCacheFile(originalReader, App.AssemblyStorage.AssemblySettings.DefaultDatabase);
					var newFile = CacheFileLoader.LoadCacheFile(newReader, App.AssemblyStorage.AssemblySettings.DefaultDatabase);
		...
		private void btnPokePatch_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				...
				if (App.AssemblyStorage.AssemblySettings.Xbdm == null || String.IsNullOrEmpty(App.AssemblyStorage.AssemblySettings.Xbdm.DeviceIdent))
				{
				...
				if (currentPatchToPoke.MetaChangesIndex >= 0)
				{
					...
					{
						App.AssemblyStorage.AssemblySettings.Xbdm.MemoryStream.Seek(currentPatchToPoke.MetaPokeBase + change.Offset, SeekOrigin.Begin);
