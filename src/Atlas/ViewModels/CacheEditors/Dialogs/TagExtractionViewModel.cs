...
namespace Atlas.ViewModels.CacheEditors.Dialogs
{
	public class TagExtractionViewModel : Base
	{
		public TagExtractionViewModel(ICacheFile cacheFile, EngineDescription engineDescription, IStreamManager streamManager, TagHierarchyNode tag)
		{
			CacheFile = cacheFile;
			...
			RelevantTags = new Dictionary<ITag, ExtractedTag>();
			...
			using (var reader = StreamManager.OpenRead())
			{
				while (TagsToProcess.Count > 0)
				{
					...
					var extractedTag = new ExtractedTag(currentTag.Index, currentTag.MetaLocation.AsPointer(), currentTag.Class.Magic,
						tagName);
					RelevantTags.Add(currentTag, extractedTag);
		...
		public Dictionary<ITag, ExtractedTag> RelevantTags
		{
			get { return _relevantTags; }
			set { SetField(ref _relevantTags, value); }
		}
		private Dictionary<ITag, ExtractedTag> _relevantTags;
