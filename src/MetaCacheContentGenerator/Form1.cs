...
namespace MetaCacheContentGenerator
{
	...
	{
		private readonly string[] blfBlockList =
		{
			"mainmenu",
			"term_",
			"m70_a"
		...
		{
			List<MetaItem> metaItems = (from ListViewItem item in lvList.Items select (MetaItem) item.Tag).ToList();
		...
		{
			string rootFolder = Path.GetDirectoryName(Application.StartupPath);
			...
			foreach (MetaItem item in items)
			{
				...
				string gameTarget = item.Targets.Split('|')[0];
				string[] files = Directory.GetFiles(item.MapInfoPath, "*.mapinfo");
				foreach (string file in files)
				{
					var fi = new FileInfo(file);
					bool continueProcessing = true;
					foreach (string block in blfBlockList.Where(block => fi.Name.StartsWith(block)))
					...
					var blfFile1 =
						new PureBLF(string.Format("{0}\\{1}{2}.blf", item.BlfPath, mapInfo.MapInformation.PhysicalName, extraSuffix));
