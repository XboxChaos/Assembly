...
namespace ExtryzeDLL.Patching
{
    public static class DataComparer
    {
        ...
        /// <param name="extraDataAtEnd">true if extra data in the modified file will be found at the end of the area to compare, or false if it will be found at the beginning.</param>
        /// <returns>The differences between the two parts of the files.</returns>
        public static List<DataChange> CompareData(IReader originalReader, uint originalOffset, int originalSize, IReader newReader, uint newOffset, int newSize, bool extraDataAtEnd)
        {
            ...
            if (sizeDiff > 0)
            {
                ...
                uint offset = (uint)(newReader.Position - newOffset);
                var data = newReader.ReadBlock(sizeDiff);
            ...
            uint originalEndOffset = (uint)(originalOffset + originalSize);
            uint newEndOffset = (uint)(newOffset + newSize);
            uint bufferOffset = (uint)(newReader.Position - newOffset);
            ...
            while (originalOffset < originalEndOffset && newOffset < newEndOffset)
            {
                ...
                var bufferLength = Math.Min(oldBuffer.Length, newBuffer.Length);
