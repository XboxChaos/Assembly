...
namespace Assembly.Backend.Plugins
{
    public class ThirdGenPluginVisitor : IPluginVisitor
    {
        ...
        private List<ReflexiveData> _reflexives = new List<ReflexiveData>();
        private ReflexiveData _currentReflexive = null;
        private bool _showInvisibles = false;

        private EnumData _currentEnum = null;
        private BitfieldData _currentBitfield = null;
        private IList<PluginRevision> _pluginRevisions = new List<PluginRevision>();
        ...
        {
            if (visible || _showInvisibles)
                AddValue(new DataRef(name, offset, "", pluginLine));
        }
        ...
        public bool EnterReflexive(string name, uint offset, bool visible, uint entrySize, uint pluginLine)
        {
            if (visible || _showInvisibles)
            {
                ReflexiveData data = new ReflexiveData(name, offset, entrySize, pluginLine);
                AddValue(data);
        ...
        public void LeaveReflexive()
        {
            ...
            if (_reflexives.Count > 0)
                _currentReflexive = _reflexives[_reflexives.Count - 1];
            else
                _currentReflexive = null;
        }
        #endregion
