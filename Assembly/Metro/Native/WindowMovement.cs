...
namespace Assembly.Metro.Native
{
    public static class WindowMovement
    {
        ...
        private static void IsDraggableChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (newValue)
            {
                ...
                if (!MakeWindowMovable(window))
                    window.SourceInitialized += WindowSourceInitialized; // The window isn't initialized, so wait for it to be before making it movable
            }
            else
            {
        ...
        public static bool HasDragsWindowEnabled(DependencyObject visual)
        {
            ...
            return (dragsWindow != DependencyProperty.UnsetValue && (bool)dragsWindow == true);
        }
        #endregion

        private static Dictionary<Window, CaptionHitTester> _registeredWindows = new Dictionary<Window, CaptionHitTester>();
